...
# How does cluster membership vary by anatomical region?
...
####  Load object from Yanwen with histological region info embedded #########
#Note: doublecheck that Cell IDs were indeed correct
UCI604_with_regions.Xenium <- readRDS(file = "/share/crsp/lab/dalawson/share/HBCA_Phase2/DeepDive_MP_JIR/Jupyter/Yanwen/UCI604.Xenium_with_hist_region.rds")

# How does cluster membership vary by histological region?
table(Idents(UCI604_with_regions.Xenium), UCI604_with_regions.Xenium$big_hist_region)
prop.table(table(Idents(UCI604_with_regions.Xenium), UCI604_with_regions.Xenium$big_hist_region), margin = 2)

Fraction <- table(Idents(UCI604_with_regions.Xenium), UCI604_with_regions.Xenium$big_hist_region)
Fraction <- as.data.frame(Fraction)
Fraction$Var1 <- as.character(Fraction$Var1)
...
####### Subset Xenium objects to Adipose, Duct, Lobular, Connective areas ############
### Note: doesnt seem to work properly yet
Ducts_UCI604_Xenium <- subset(UCI604_with_regions.Xenium, subset = big_hist_region == "duct") # gives me error that no cells were found
Lobule_UCI604_Xenium <- subset(UCI604_with_regions.Xenium, subset = big_hist_region == "lobule") # gives warnings
Adipose_UCI604_Xenium <- subset(UCI604_with_regions.Xenium, subset = big_hist_region == "adipose") # gives warnings
Connective_UCI604_Xenium <- subset(UCI604_with_regions.Xenium, subset = big_hist_region == "connective_tissue") # gives warnings

ImageDimPlot(Lobule_UCI604.Xenium, fov = "P2.LIQ", molecules =  c("MYLK", "KRT8", "LUM", "ADIPOQ"), 
             nmols=20000, axes = TRUE) # doesn't seem to work; I would potentially also like to plot select cell types instead of molecules later on; how do I define this in the code?

# How does cluster membership vary by histological region? --> Lobule
table(Idents(Lobule_UCI604_Xenium), Lobule_UCI604_Xenium$region)
prop.table(table(Idents(Lobule_UCI604_Xenium), Lobule_UCI604_Xenium$region), margin = 2)

Fraction <- table(Idents(Lobule_UCI604_Xenium), Lobule_UCI604_Xenium$region)
Fraction <- as.data.frame(Fraction)
Fraction$Var1 <- as.character(Fraction$Var1)
...
# How does cluster membership vary by histological region? --> Duct
table(Idents(Duct_UCI604_Xenium), Duct_UCI604_Xenium$region)
prop.table(table(Idents(Duct_UCI604_Xenium), Duct_UCI604_Xenium$region), margin = 2)

Fraction <- table(Idents(Duct_UCI604_Xenium), Duct_UCI604_Xenium$region)
Fraction <- as.data.frame(Fraction)
Fraction$Var1 <- as.character(Fraction$Var1)

### stacked barplot 
ggplot(Fraction, aes(x = Var2, y = Freq, fill = Var1)) + ## Note: are there any options to make colors better distinguished in stacked barplots? 
  theme_bw(base_size = 15) +
  geom_col(position = "fill", width = 0.7) +
  xlab("Sample") +
  ylab("Proportion") +
...
# How does cluster membership vary by histological region? --> Adipose
table(Idents(Adipose_UCI604_Xenium), Adipose_UCI604_Xenium$region)
prop.table(table(Idents(Adipose_UCI604_Xenium), Adipose_UCI604_Xenium$region), margin = 2)

Fraction <- table(Idents(Adipose_UCI604_Xenium), Adipose_UCI604_Xenium$region)
Fraction <- as.data.frame(Fraction)
Fraction$Var1 <- as.character(Fraction$Var1)
...
# How does cluster membership vary by histological region? --> Connective Tissue
table(Idents(Connective_UCI604_Xenium), Connective_UCI604_Xenium$region)
prop.table(table(Idents(Connective_UCI604_Xenium), Connective_UCI604_Xenium$region), margin = 2)

Fraction <- table(Idents(Connective_UCI604_Xenium), Connective_UCI604_Xenium$region)
Fraction <- as.data.frame(Fraction)
Fraction$Var1 <- as.character(Fraction$Var1)
...
## Questions: Instead of stacked barplots, could I also plot pie charts for each individual sample?
## Is it also possible to plot the % of a specific cell type (e.g. T cell) by histological region and/or anatomical region (=sample, e.g. P2-LIQ, P2-UOQ,...)?